...
namespace CarSpec.Services.Obd
{
    ...
    public class Elm327Adapter : IDisposable
    {
        ...
        public async Task<bool> ConnectAsync(CancellationToken ct)
        {
            ...
            try
            {
                ...
                string[] baseCmds = { "ATZ", "ATE0", "ATL0", "ATS0" };
                ...
                string[] protocolSequence = { "ATSP0", "ATSP3" };
                foreach (var proto in protocolSequence)
                {
                    ...
                    if (proto == "ATSP3")
                    {
                        ...
                        _ = await SendCommandAsync("ATSI", ct: ct);
                        await Task.Delay(3000, ct);
                        await SendCommandAsync("ATST FF", ct: ct);
        ...
        public bool SupportsPid(string pidHex)
        {
            ...
            int blockIndex = (pidVal - 1) / 0x20;
            int withinBlock = ((pidVal - 1) % 0x20) + 1;
            string key = "01" + (blockIndex * 0x20).ToString("X2");
    ...
    CancellationToken ct = default)
        ...
        private async Task ApplyProfileHintsAsync(VehicleProfile? profile, CancellationToken ct)
        {
            ...
            var hint = profile?.ProtocolHint;
            var proto = hint?.ToUpperInvariant() ?? "AUTO";

            _isIso9141 = proto.Contains("9141") || proto.Contains("14230") || proto.Contains("KWP");

            if (proto.Contains("9141"))
                await SendCommandAsync("ATSP3", ct: ct);
            else if (proto.Contains("CAN29") || proto.Contains("29/500"))
                await SendCommandAsync("ATSP7", ct: ct);
            else if (proto.Contains("CAN11") || proto.Contains("11/500") || proto.Contains("15765"))
                await SendCommandAsync("ATSP6", ct: ct);
            else
                await SendCommandAsync("ATSP0", ct: ct);

            await SendCommandAsync("ATE0", ct: ct);
            await SendCommandAsync("ATL0", ct: ct);
            await SendCommandAsync("ATS0", ct: ct);
            await SendCommandAsync("ATH0", ct: ct);
            await SendCommandAsync("ATAT2", ct: ct);

            if (_isIso9141)
            {
                await SendCommandAsync("ATST 32", ct: ct);
                _pidTimeoutMs = 1100;
            }
            else
            {
                await SendCommandAsync("ATST 0A", ct: ct);
                _pidTimeoutMs = 500;
            }
        ...
        private void LogPidSupportFrom4100(string cleaned4100)
        {
            try
            {
                ...
                bool supports010C = (map & (1u << (32 - 0x0C))) != 0;
                bool supports010D = (map & (1u << (32 - 0x0D))) != 0;
